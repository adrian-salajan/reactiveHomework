...
class BinaryTreeSuite(_system: ActorSystem) extends TestKit(_system) with FunSuiteLike with Matchers with BeforeAndAfterAll with ImplicitSender
{
  ...
  test("insert contains") {
    ...
    topNode ! Insert(testActor, id = 100, 10)
    expectMsg(FiniteDuration(5, TimeUnit.MINUTES), OperationFinished(100))


    topNode ! Insert(testActor, id = 200, 20)
    expectMsg(FiniteDuration(5, TimeUnit.MINUTES), OperationFinished(200))

    topNode ! Insert(testActor, id = 300, 5)
    expectMsg(FiniteDuration(5, TimeUnit.MINUTES), OperationFinished(300))


    topNode ! Contains(testActor, id = 400, 20)
    expectMsg(FiniteDuration(5, TimeUnit.MINUTES), ContainsResult(400, true))

    topNode ! Contains(testActor, id = 500, 5)
    expectMsg(FiniteDuration(5, TimeUnit.MINUTES), ContainsResult(500, true))

    topNode ! Contains(testActor, id = 600, 3)
    expectMsg(FiniteDuration(5, TimeUnit.MINUTES), ContainsResult(600, false))

    topNode ! Contains(testActor, id = 700, 15)
    expectMsg(FiniteDuration(5, TimeUnit.MINUTES), ContainsResult(700, false))

    topNode ! Contains(testActor, id =800, 10)
    expectMsg(FiniteDuration(5, TimeUnit.MINUTES), ContainsResult(800, true))

  }

  test("insert remove left") {
    val topNode = system.actorOf(Props[BinaryTreeSet])
    ...
    topNode ! Remove(testActor, id = 400, 5)
    expectMsg(FiniteDuration(5, TimeUnit.MINUTES), OperationFinished(400))

    topNode ! Contains(testActor, id =800, 5)
    expectMsg(FiniteDuration(5, TimeUnit.MINUTES), ContainsResult(800, false))
  }

  test("insert remove right") {
    val topNode = system.actorOf(Props[BinaryTreeSet])
    ...
    topNode ! Remove(testActor, id = 400, 20)
    expectMsg(FiniteDuration(5, TimeUnit.MINUTES), OperationFinished(400))

    topNode ! Contains(testActor, id =800, 20)
    expectMsg(FiniteDuration(5, TimeUnit.MINUTES), ContainsResult(800, false))
  }
  ...
  test("GC: same tree after complete") {
    val topNode = system.actorOf(Props[BinaryTreeSet])
    ...
    topNode ! Contains(testActor, id =400, 10)
    expectMsg(FiniteDuration(5, TimeUnit.MINUTES), ContainsResult(400, true))
    topNode ! Contains(testActor, id =401, 20)
    expectMsg(FiniteDuration(5, TimeUnit.MINUTES), ContainsResult(401, true))
    topNode ! Contains(testActor, id =402, 5)
    expectMsg(FiniteDuration(5, TimeUnit.MINUTES), ContainsResult(402, true))
  }
