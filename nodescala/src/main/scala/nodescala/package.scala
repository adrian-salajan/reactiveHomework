...
package object nodescala {
  ...
  implicit class FutureCompanionOps(val f: Future.type) extends AnyVal {
    ...
    def all[T](fs: List[Future[T]]): Future[List[T]] = fs match {
      ...
      case futureElement :: listFuture => futureElement.flatMap(e => all(listFuture).flatMap(li => Future(e :: li)))
    }
    ...
    def any[T](fs: List[Future[T]]): Future[T] = {
      ...
      if (fs.isEmpty) promise.failure(new Exception("no future in list"))
      else fs.foreach(f => f.onComplete(promise.tryComplete(_)))
      promise.future
    }
  ...
  implicit class FutureOps[T](val f: Future[T]) extends AnyVal {
    ...
    def now: T = {
      ...
      } else throw new NoSuchElementException("future is not yet completed")
    }
    ...
    def continueWith[S](cont: Future[T] => S): Future[S] = {
      ...
      f.onComplete( t => t match {
        case Success(e) => p.complete(Try(cont(f)))
    ...
    def continue[S](cont: Try[T] => S): Future[S] = {
      val p = Promise[S]()
      f.onComplete( t => t match {
        case Success(e) => p.complete(Try(cont(t)))
        case Failure(f) => p.failure(f)
      })
      p.future
    }
