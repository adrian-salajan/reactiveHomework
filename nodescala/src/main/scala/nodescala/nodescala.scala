...
trait NodeScala {
  ...
  private def respond(exchange: Exchange, token: CancellationToken, response: Response): Unit = {
    while (token.nonCancelled) {
      if (response.hasNext) {
        val responsePart = response.next
        println("writing: " + responsePart)
        exchange.write(responsePart)
      } else {
        exchange.close()
      }
    }
  ...
  def start(relativePath: String)(handler: Request => Response): Subscription = {
    ...
    Future.run()((ct:CancellationToken) => Future {
      while(!ct.isCancelled) {
        val request = listener.nextRequest()
        request.onComplete(t => t match {
          case Failure(e) => {println("request ended in failure")}
          case Success((req:Request, e:Exchange)) => {
            Future {
              respond(e, ct, handler(req))
            }
          }
        })
      }
