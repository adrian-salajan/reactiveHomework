...
object WikipediaSuggest extends SimpleSwingApplication with ConcreteSwingApi with ConcreteWikipediaApi {
  ...
  def top = new MainFrame {
    ...
    val suggestions: Observable[Try[List[String]]] = {
     val x = //searchTerms.concatRecovered(wikiSuggestResponseStream)
       searchTerms.concatRecovered(wikiSuggestResponseStream)
      x.foreach(
        s => println("success " + {s match {
          case Failure(e: RetrofitError) => println(e.getUrl + " " + e.isNetworkError + " " + e.getBody)
    ...
    val suggestionSubscription: Subscription =  suggestions.observeOn(eventScheduler) subscribe {
      x => x match {
        case Success(v :List[String]) =>{
          suggestionList.listData = v
        }
        case Failure(f) => {
          suggestionList.listData = List(f.getMessage)
          System.err.print(f)
        }
      }
    }
    ...
    val selections: Observable[String] = {
      button.clicks.filter(b => !suggestionList.selection.items.isEmpty)
         .flatMap(b => Observable.from(suggestionList.selection.items))
    }
    ...
    val pageSubscription: Subscription = pages.observeOn(eventScheduler) subscribe {
      x => x match {
        ...
        case Failure(f) => editorpane.text = f.getMessage
...
trait ConcreteWikipediaApi extends WikipediaApi {
  def wikipediaSuggestion(term: String) = {println("ttterm: " + term); Search.wikipediaSuggestion(term)}
