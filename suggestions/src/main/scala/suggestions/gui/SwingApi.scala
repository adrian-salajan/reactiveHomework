...
trait SwingApi {
  ...
  implicit class TextFieldOps(field: TextField) {
    ...
    def textValues: Observable[String] = Observable.create(
      obs => {
        ...
        val textChangedReaction: Reaction = {case e: ValueChanged => obs.onNext(field.text)}

        field.subscribe(
          textChangedReaction
        )

        Subscription.apply(field.unsubscribe(textChangedReaction))
      }
  ...
  implicit class ButtonOps(button: Button) {
    ...
    def clicks: Observable[Button] = Observable.create( obs => {
      val reaction: Reaction = {case e: ButtonClicked => obs.onNext(button)}
      button.subscribe(reaction)
      Subscription.apply{button.unsubscribe(reaction)}
    })
  }
