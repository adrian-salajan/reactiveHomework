...
class SwingApiTest extends FunSuite {
  ...
  test("SwingApi should emit text field values to the observable, while subscribed") {
    val textField = new swingApi.TextField
    val values = textField.textValues
    ...
    assert(observed2 == Seq("a", "b"), observed2)
  }

  test("clicks should return stream of clicked buttons") {
    ...
    assert(observed == Seq(button, button), observed)


  }

  test("SwingApi should emit text field values to the observable2") {
    val textField = new swingApi.TextField
    val values = textField.textValues
